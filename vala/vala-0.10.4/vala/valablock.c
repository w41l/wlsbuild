...
void vala_block_add_statement (ValaBlock* self, ValaStatement* stmt) {
	...
	vala_collection_add ((ValaCollection*) self->priv->statement_list, stmt);
}
...
void vala_block_insert_statement (ValaBlock* self, gint index, ValaStatement* stmt) {
	...
	g_return_if_fail (stmt != NULL);
	vala_code_node_set_parent_node ((ValaCodeNode*) stmt, (ValaCodeNode*) self);
	vala_list_insert (self->priv->statement_list, index, stmt);
}
...
ValaList* vala_block_get_statements (ValaBlock* self) {
	...
	list = vala_array_list_new (VALA_TYPE_STATEMENT, (GBoxedCopyFunc) vala_code_node_ref, vala_code_node_unref, g_direct_equal);
	{
		ValaIterator* _stmt_it;
		_stmt_it = vala_iterable_iterator ((ValaIterable*) self->priv->statement_list);
		while (TRUE) {
			ValaStatement* stmt;
			...
			stmt_list = _vala_code_node_ref0 ((_tmp0_ = stmt, VALA_IS_STATEMENT_LIST (_tmp0_) ? ((ValaStatementList*) _tmp0_) : NULL));
			if (stmt_list != NULL) {
				{
					gint i;
					i = 0;
					{
						...
						while (TRUE) {
							...
							_tmp1_ = FALSE;
							if (!(i < vala_statement_list_get_length (stmt_list))) {
								break;
							}
							vala_collection_add ((ValaCollection*) list, _tmp2_ = vala_statement_list_get (stmt_list, i));
							_vala_code_node_unref0 (_tmp2_);
						}
					}
				}
			} else {
				vala_collection_add ((ValaCollection*) list, stmt);
			}
...
void vala_block_add_local_variable (ValaBlock* self, ValaLocalVariable* local) {
	...
	parent_block = _vala_code_node_ref0 (vala_symbol_get_parent_symbol ((ValaSymbol*) self));
	while (TRUE) {
		...
		if (VALA_IS_BLOCK (parent_block)) {
			...
			_tmp1_ = VALA_IS_METHOD (parent_block);
		}
		if (_tmp1_) {
			_tmp0_ = TRUE;
		} else {
			_tmp0_ = VALA_IS_PROPERTY_ACCESSOR (parent_block);
		}
		if (!_tmp0_) {
			break;
		}
		if ((_tmp3_ = (_tmp2_ = vala_scope_lookup (vala_symbol_get_scope (parent_block), vala_symbol_get_name ((ValaSymbol*) local))) != NULL, _vala_code_node_unref0 (_tmp2_), _tmp3_)) {
			char* _tmp4_;
			vala_report_error (vala_code_node_get_source_reference ((ValaCodeNode*) local), _tmp4_ = g_strdup_printf ("Local variable `%s' conflicts with another local variable declared in " \
"a parent scope", vala_symbol_get_name ((ValaSymbol*) local)));
			_g_free0 (_tmp4_);
			break;
		}
		parent_block = (_tmp5_ = _vala_code_node_ref0 (vala_symbol_get_parent_symbol (parent_block)), _vala_code_node_unref0 (parent_block), _tmp5_);
	}
	vala_collection_add ((ValaCollection*) self->priv->local_variables, local);
...
void vala_block_remove_local_variable (ValaBlock* self, ValaLocalVariable* local) {
	g_return_if_fail (self != NULL);
	g_return_if_fail (local != NULL);
	vala_collection_remove ((ValaCollection*) self->priv->local_variables, local);
}
...
ValaList* vala_block_get_local_variables (ValaBlock* self) {
	ValaList* result = NULL;
	g_return_val_if_fail (self != NULL, NULL);
	result = _vala_collection_object_ref0 (self->priv->local_variables);
	return result;
}
...
static gboolean vala_block_real_check (ValaCodeNode* base, ValaSemanticAnalyzer* analyzer) {
	...
	analyzer->insert_block = (_tmp0_ = _vala_code_node_ref0 (self), _vala_code_node_unref0 (analyzer->insert_block), _tmp0_);
	{
		gint i;
		i = 0;
		{
			...
			while (TRUE) {
				...
				_tmp1_ = FALSE;
				if (!(i < vala_collection_get_size ((ValaCollection*) self->priv->statement_list))) {
					break;
				}
				vala_code_node_check ((ValaCodeNode*) (_tmp2_ = (ValaStatement*) vala_list_get (self->priv->statement_list, i)), analyzer);
				_vala_code_node_unref0 (_tmp2_);
			}
		}
	}
	{
		...
		_local_it = (_tmp4_ = vala_iterable_iterator ((ValaIterable*) (_tmp3_ = vala_block_get_local_variables (self))), _vala_collection_object_unref0 (_tmp3_), _tmp4_);
		while (TRUE) {
	...
	}
	{
		...
		ValaIterator* _tmp6_;
		ValaIterator* _stmt_it;
		_stmt_it = (_tmp6_ = vala_iterable_iterator ((ValaIterable*) (_tmp5_ = vala_block_get_statements (self))), _vala_collection_object_unref0 (_tmp5_), _tmp6_);
		while (TRUE) {
			ValaStatement* stmt;
			ValaList* _tmp7_;
			if (!vala_iterator_next (_stmt_it)) {
			...
			stmt = (ValaStatement*) vala_iterator_get (_stmt_it);
			vala_code_node_add_error_types ((ValaCodeNode*) self, _tmp7_ = vala_code_node_get_error_types ((ValaCodeNode*) stmt));
	...
	analyzer->insert_block = (_tmp8_ = _vala_code_node_ref0 (old_insert_block), _vala_code_node_unref0 (analyzer->insert_block), _tmp8_);
	result = !vala_code_node_get_error ((ValaCodeNode*) self);
	...
	g_return_if_fail (new_stmt != NULL);
	{
		...
		i = 0;
		{
			...
			while (TRUE) {
				...
				stmt_list = (_tmp1_ = (ValaStatement*) vala_list_get (self->priv->statement_list, i), VALA_IS_STATEMENT_LIST (_tmp1_) ? ((ValaStatementList*) _tmp1_) : NULL);
				if (stmt_list != NULL) {
					{
						gint j;
						j = 0;
						{
							...
							while (TRUE) {
								...
								_tmp2_ = FALSE;
								if (!(j < vala_statement_list_get_length (stmt_list))) {
									break;
								}
								if ((_tmp4_ = (_tmp3_ = vala_statement_list_get (stmt_list, j)) == stmt, _vala_code_node_unref0 (_tmp3_), _tmp4_)) {
					...
					gboolean _tmp6_;
					if ((_tmp6_ = (_tmp5_ = (ValaStatement*) vala_list_get (self->priv->statement_list, i)) == stmt, _vala_code_node_unref0 (_tmp5_), _tmp6_)) {
						ValaStatementList* _tmp7_;
						stmt_list = (_tmp7_ = vala_statement_list_new (vala_code_node_get_source_reference ((ValaCodeNode*) self)), _vala_code_node_unref0 (stmt_list), _tmp7_);
						vala_statement_list_add (stmt_list, new_stmt);
						vala_statement_list_add (stmt_list, stmt);
						vala_list_set (self->priv->statement_list, i, (ValaStatement*) stmt_list);
					}
				}
				_vala_code_node_unref0 (stmt_list);
			}
				...
				if (stmt_list != NULL) {
					{
						...
						j = 0;
						{
							...
							while (TRUE) {
								...
								if ((_tmp4_ = (_tmp3_ = vala_statement_list_get (stmt_list, j)) == old_stmt, _vala_code_node_unref0 (_tmp3_), _tmp4_)) {
					...
					ValaStatement* _tmp5_;
					gboolean _tmp6_;
					if ((_tmp6_ = (_tmp5_ = (ValaStatement*) vala_list_get (self->priv->statement_list, i)) == old_stmt, _vala_code_node_unref0 (_tmp5_), _tmp6_)) {
						vala_list_set (self->priv->statement_list, i, new_stmt);
						_vala_code_node_unref0 (stmt_list);
						break;
					}
				}
...
static void vala_block_instance_init (ValaBlock * self) {
	...
	self->priv->statement_list = (ValaList*) vala_array_list_new (VALA_TYPE_STATEMENT, (GBoxedCopyFunc) vala_code_node_ref, vala_code_node_unref, g_direct_equal);
	self->priv->local_variables = (ValaList*) vala_array_list_new (VALA_TYPE_LOCAL_VARIABLE, (GBoxedCopyFunc) vala_code_node_ref, vala_code_node_unref, g_direct_equal);
}
...
static void vala_block_finalize (ValaCodeNode* obj) {
	ValaBlock * self;
	self = VALA_BLOCK (obj);
	_vala_collection_object_unref0 (self->priv->statement_list);
	_vala_collection_object_unref0 (self->priv->local_variables);
