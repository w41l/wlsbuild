...
void vala_switch_section_add_label (ValaSwitchSection* self, ValaSwitchLabel* label) {
	...
	vala_collection_add ((ValaCollection*) self->priv->labels, label);
...
gboolean vala_switch_section_has_default_label (ValaSwitchSection* self) {
	...
	g_return_val_if_fail (self != NULL, FALSE);
	{
		...
		_label_it = vala_iterable_iterator ((ValaIterable*) self->priv->labels);
		while (TRUE) {
...
static void vala_switch_section_real_accept_children (ValaCodeNode* base, ValaCodeVisitor* visitor) {
	...
	}
	{
		...
		ValaIterator* _tmp1_;
		ValaIterator* _st_it;
		_st_it = (_tmp1_ = vala_iterable_iterator ((ValaIterable*) (_tmp0_ = vala_block_get_statements ((ValaBlock*) self))), _vala_collection_object_unref0 (_tmp0_), _tmp1_);
		while (TRUE) {
			ValaStatement* st;
			if (!vala_iterator_next (_st_it)) {
				break;
			}
			st = (ValaStatement*) vala_iterator_get (_st_it);
...
static gboolean vala_switch_section_real_check (ValaCodeNode* base, ValaSemanticAnalyzer* analyzer) {
	...
	vala_code_node_set_checked ((ValaCodeNode*) self, TRUE);
	{
		...
		ValaIterator* _tmp1_;
		ValaIterator* _label_it;
		_label_it = (_tmp1_ = vala_iterable_iterator ((ValaIterable*) (_tmp0_ = vala_switch_section_get_labels (self))), _vala_collection_object_unref0 (_tmp0_), _tmp1_);
		while (TRUE) {
			ValaSwitchLabel* label;
			if (!vala_iterator_next (_label_it)) {
				break;
	...
	analyzer->insert_block = (_tmp2_ = _vala_code_node_ref0 ((ValaBlock*) self), _vala_code_node_unref0 (analyzer->insert_block), _tmp2_);
	{
		ValaList* _tmp3_;
		ValaIterator* _tmp4_;
		ValaIterator* _st_it;
		_st_it = (_tmp4_ = vala_iterable_iterator ((ValaIterable*) (_tmp3_ = vala_block_get_statements ((ValaBlock*) self))), _vala_collection_object_unref0 (_tmp3_), _tmp4_);
		while (TRUE) {
			ValaStatement* st;
			if (!vala_iterator_next (_st_it)) {
				break;
...
static void vala_switch_section_real_emit (ValaCodeNode* base, ValaCodeGenerator* codegen) {
	...
	g_return_if_fail (codegen != NULL);
	{
		...
		while (TRUE) {
			...
			label = (ValaSwitchLabel*) vala_iterator_get (_label_it);
			vala_code_node_emit ((ValaCodeNode*) label, codegen);
			_vala_code_node_unref0 (label);
		}
		_vala_collection_object_unref0 (_label_it);
	}
	VALA_CODE_NODE_CLASS (vala_switch_section_parent_class)->emit ((ValaCodeNode*) VALA_BLOCK (self), codegen);
}
...
static void vala_switch_section_instance_init (ValaSwitchSection * self) {
	...
	self->priv->labels = (ValaList*) vala_array_list_new (VALA_TYPE_SWITCH_LABEL, (GBoxedCopyFunc) vala_code_node_ref, vala_code_node_unref, g_direct_equal);
}
...
static void vala_switch_section_finalize (ValaCodeNode* obj) {
	...
	_vala_collection_object_unref0 (self->priv->labels);
