...
static char* vala_regex_literal_real_to_string (ValaCodeNode* base);
const char* vala_regex_literal_get_value (ValaRegexLiteral* self);
static gboolean vala_regex_literal_real_check (ValaCodeNode* base, ValaSemanticAnalyzer* analyzer);
...
static gboolean vala_regex_literal_real_check (ValaCodeNode* base, ValaSemanticAnalyzer* analyzer) {
	...
	if (!vala_code_context_get_experimental (vala_semantic_analyzer_get_context (analyzer))) {
		vala_report_warning (vala_code_node_get_source_reference ((ValaCodeNode*) self), "regular expression literals are experimental");
	}
	{
		GRegex* regex;
		regex = g_regex_new (self->priv->_value, 0, 0, &_inner_error_);
	...
	__catch8_g_regex_error:
	{
		...
		_inner_error_ = NULL;
		{
			...
			vala_report_error (vala_code_node_get_source_reference ((ValaCodeNode*) self), _tmp0_ = g_strdup_printf ("Invalid regular expression `%s'.", self->priv->_value));
			_g_free0 (_tmp0_);
			result = FALSE;
			_g_error_free0 (err);
			return result;
	...
	vala_expression_set_value_type ((ValaExpression*) self, _tmp1_ = vala_data_type_copy (analyzer->regex_type));
