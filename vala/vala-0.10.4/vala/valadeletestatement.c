...
static gboolean vala_delete_statement_real_check (ValaCodeNode* base, ValaSemanticAnalyzer* analyzer) {
	...
	if (!VALA_IS_POINTER_TYPE (vala_expression_get_value_type (self->priv->_expression))) {
		...
		vala_report_error (vala_code_node_get_source_reference ((ValaCodeNode*) self), _tmp1_ = g_strdup_printf ("delete operator not supported for `%s'", _tmp0_ = vala_code_node_to_string ((ValaCodeNode*) vala_expression_get_value_type (self->priv->_expression))));
		_g_free0 (_tmp1_);
		_g_free0 (_tmp0_);
	}
	result = !vala_code_node_get_error ((ValaCodeNode*) self);
...
static void vala_delete_statement_real_emit (ValaCodeNode* base, ValaCodeGenerator* codegen) {
	...
	vala_code_node_emit ((ValaCodeNode*) self->priv->_expression, codegen);
