...
static char* vala_integer_literal_real_to_string (ValaCodeNode* base) {
	...
	char* result = NULL;
	self = (ValaIntegerLiteral*) base;
	result = g_strdup (self->priv->_value);
	return result;
}
...
static gboolean vala_integer_literal_real_check (ValaCodeNode* base, ValaSemanticAnalyzer* analyzer) {
	...
	while (TRUE) {
		...
		char* _tmp1_;
		if (g_str_has_suffix (self->priv->_value, "l")) {
			_tmp0_ = TRUE;
		} else {
			_tmp0_ = g_str_has_suffix (self->priv->_value, "L");
		}
		if (!_tmp0_) {
			break;
		}
		l++;
		vala_integer_literal_set_value (self, _tmp1_ = g_strndup (self->priv->_value, strlen (self->priv->_value) - 1));
		_g_free0 (_tmp1_);
	}
	u = FALSE;
	if (g_str_has_suffix (self->priv->_value, "u")) {
		_tmp2_ = TRUE;
	} else {
		_tmp2_ = g_str_has_suffix (self->priv->_value, "U");
	}
	if (_tmp2_) {
		char* _tmp3_;
		u = TRUE;
		vala_integer_literal_set_value (self, _tmp3_ = g_strndup (self->priv->_value, strlen (self->priv->_value) - 1));
		_g_free0 (_tmp3_);
	}
	n = g_ascii_strtoll (self->priv->_value, NULL, 0);
	if (!u) {
		_tmp4_ = n > 0x7fffffff;
	} else {
		_tmp4_ = FALSE;
	...
	if (l == 0) {
		if (u) {
			...
			type_name = (_tmp6_ = g_strdup ("uint"), _g_free0 (type_name), _tmp6_);
		} else {
			char* _tmp7_;
			vala_integer_literal_set_type_suffix (self, "");
			type_name = (_tmp7_ = g_strdup ("int"), _g_free0 (type_name), _tmp7_);
		}
	} else {
		if (l == 1) {
			if (u) {
				...
				type_name = (_tmp8_ = g_strdup ("ulong"), _g_free0 (type_name), _tmp8_);
			} else {
				char* _tmp9_;
				vala_integer_literal_set_type_suffix (self, "L");
				type_name = (_tmp9_ = g_strdup ("long"), _g_free0 (type_name), _tmp9_);
			}
		} else {
			ValaCodeContext* _tmp10_;
			gboolean _tmp11_;
			if ((_tmp11_ = vala_code_context_get_profile (_tmp10_ = vala_code_context_get ()) == VALA_PROFILE_DOVA, _vala_code_context_unref0 (_tmp10_), _tmp11_)) {
				if (u) {
					char* _tmp12_;
					vala_integer_literal_set_type_suffix (self, "UL");
					type_name = (_tmp12_ = g_strdup ("ulong"), _g_free0 (type_name), _tmp12_);
				} else {
					char* _tmp13_;
					vala_integer_literal_set_type_suffix (self, "L");
					type_name = (_tmp13_ = g_strdup ("long"), _g_free0 (type_name), _tmp13_);
				}
			} else {
				if (u) {
					char* _tmp14_;
					vala_integer_literal_set_type_suffix (self, "ULL");
					type_name = (_tmp14_ = g_strdup ("uint64"), _g_free0 (type_name), _tmp14_);
				} else {
					char* _tmp15_;
					vala_integer_literal_set_type_suffix (self, "LL");
					type_name = (_tmp15_ = g_strdup ("int64"), _g_free0 (type_name), _tmp15_);
				}
			}
		}
	}
	st = VALA_STRUCT (vala_scope_lookup (vala_symbol_get_scope (analyzer->root_symbol), type_name));
	vala_code_node_check ((ValaCodeNode*) st, analyzer);
	vala_expression_set_value_type ((ValaExpression*) self, (ValaDataType*) (_tmp16_ = vala_integer_type_new (st, self->priv->_value, type_name)));
...
void vala_integer_literal_set_type_suffix (ValaIntegerLiteral* self, const char* value) {
	char* _tmp0_;
	g_return_if_fail (self != NULL);
	self->priv->_type_suffix = (_tmp0_ = g_strdup (value), _g_free0 (self->priv->_type_suffix), _tmp0_);
}
